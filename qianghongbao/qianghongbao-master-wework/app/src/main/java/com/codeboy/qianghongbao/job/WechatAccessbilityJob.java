...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    /** 微信的包名*/
    public static final String WECHAT_PACKAGENAME = "com.tencent.wework";

    /** 红包消息的关键字*/
    private static final String HONGBAO_TEXT_KEY = "[拼手气红包]";
    ...
    private boolean isMemberChatUi(AccessibilityNodeInfo nodeInfo) {
        ...
        String id = "com.tencent.wework:id/ces";
        int wv = getWechatVersion();
        if(wv <= 680) {
            id = "com.tencent.wework:id/ew";
        } else if(wv <= 700) {
            id = "com.tencent.wework:id/cbo";
        }
        String title = null;
    ...
    private void openHongBao(AccessibilityEvent event) {
        ...
        if("com.tencent.wework.enterprise.redenvelopes.controller.RedEnvelopeCollectorWithCoverActivity".equals(event.getClassName())) {
            mCurrentWindow = WINDOW_LUCKYMONEY_RECEIVEUI;
            //点中了红包，下一步就是去拆红包
            handleLuckyMoneyReceive();
        } else if("com.tencent.wework.enterprise.redenvelopes.controller.RedEnvelopeCollectorActivity".equals(event.getClassName())
                    || "com.tencent.wework.enterprise.redenvelopes.controller.RedEnvelopeDetailActivity".equals(event.getClassName())
                    || "com.tencent.wework.enterprise.redenvelopes.controller.RedEnvelopeDetailWithCoverActivity".equals(event.getClassName())) {
        ...
        } else if("com.tencent.wework.ui.LauncherUI".equals(event.getClassName())
                || "com.tencent.wework.launch.WwMainActivity".equals(event.getClassName())
                || "com.tencent.wework.msg.controller.MessageListActivity".equals(event.getClassName())) {
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(event == Config.WX_AFTER_OPEN_HONGBAO) { //拆红包
            if (wechatVersion < USE_ID_MIN_VERSION) {
                ...
                String buttonId = "com.tencent.wework:id/fyy";

                if(wechatVersion == 700) {
                    buttonId = "com.tencent.wework:id/b2c";
                }
    ...
    private void handleChatListHongBao() {
        ...
        if(listNotReceived != null && listNotReceived.isEmpty()) {
            // 从消息列表查找红包
            AccessibilityNodeInfo nodeChat = AccessibilityHelper.findNodeInfosByIdFilterByText(nodeInfo,"com.tencent.wework:id/ht0", "[拼手气红包]"); //进入聊天界面
            if(nodeChat != null) {
