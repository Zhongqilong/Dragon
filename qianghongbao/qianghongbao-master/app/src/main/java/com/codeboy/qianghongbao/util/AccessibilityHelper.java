...
public final class AccessibilityHelper {
    ...
    public static AccessibilityNodeInfo findNodeInfosByClassNameFilterByDesc(AccessibilityNodeInfo nodeInfo, String className, String desc) {
        if(nodeInfo == null){
            return null;
        }
        ...
        for (int i = 0; i < nodeInfo.getChildCount(); i++) {
            AccessibilityNodeInfo node = nodeInfo.getChild(i);
            Log.d("test-------", "findNodeInfosByClassNameFilterByDesc:---------- " + node.getViewIdResourceName());
            if(className.equals(node.getClassName())) {
                String str = String.valueOf(node.getContentDescription());
                if (desc.equals(str)) {
                    return node;
                }
            }
            else if(node.getChildCount() > 0) {
                return findNodeInfosByClassNameFilterByDesc(node, className, desc);
            }
        }
        return null;
    }
