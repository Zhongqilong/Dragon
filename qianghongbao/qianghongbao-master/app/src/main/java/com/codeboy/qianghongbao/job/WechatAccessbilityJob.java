...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    private boolean isOpeningHongbao = false;
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(event == Config.WX_AFTER_OPEN_HONGBAO) { //拆红包
            if (wechatVersion < USE_ID_MIN_VERSION) {
                ...
                targetNode = AccessibilityHelper.findNodeInfosById(nodeInfo, "com.tencent.mm:id/den");
                if(targetNode == null) {
                    //分别对应固定金额的红包 拼手气红包
        ...
        if(targetNode != null) {
            ...
            //红包已经抢完
            AccessibilityNodeInfo getNode = AccessibilityHelper.findNodeInfosByText(nodeInfo, "手慢了，红包派完了");
            if(getNode != null){
                //已经领完了
                AccessibilityHelper.performBack(getService());
            }
    ...
    private void handleChatListHongBao() {
        ...
        if(listNotReceived != null && listNotReceived.isEmpty()) {
            ...
            AccessibilityHelper.performClick(node); //打开红包
        }
    }
