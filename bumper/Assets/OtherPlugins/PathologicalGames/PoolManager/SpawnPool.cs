    ...
    public partial class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        private void Awake()
        {
            ...
            for (int i = 0; i < this._perPrefabPoolOptions.Count; i++)
            {
                ...
                {
                    UnityEngine.Debug.LogWarning(string.Format("Initialization Warning: Pool '{0}' " +
        ...
        private void OnDestroy()
        {
            ...
            // Clean-up
            int cnt = _prefabPools.Count;
            for(int i=0; i<cnt; i++)
            {
                _prefabPools[i].SelfDestruct();
            }
        ...
        public void Add(Transform instance, string prefabName, bool despawn, bool parent)
        {
            ...
            UnityEngine.Debug.LogError(string.Format("SpawnPool {0}: PrefabPool {1} not found.",
                                         this.poolName,
                                         prefabName));
        ...
        public void Despawn(string prefabName, Transform instance)
        {
            // Find the item and despawn it
            bool despawned = false;
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                if (this._prefabPools[i].name == prefabName)
                {
            ...
            if (!despawned)
            {
                UnityEngine.Debug.LogError(string.Format("SpawnPool {0}: {1} not found in SpawnPool",
                               this.poolName,
                               instance.name));
                return;
            }
        ...
        {
            for (int i = this._prefabPools.Count - 1; i >= 0; i--)
            {
                if (this._prefabPools[i].prefabGO == null)
                {
                    UnityEngine.Debug.LogWarning(string.Format("SpawnPool {0}: PrefabPool.prefabGO is null",
                                                 this._prefabPools[i].name));
                    this.prefabs._Remove(this._prefabPools[i].name);
        ...
        public PrefabPool GetPrefabPool(GameObject prefab)
        {
            ...
            {
                if (this._prefabPools[i].prefabGO == null)
                    UnityEngine.Debug.LogError(string.Format("SpawnPool {0}: PrefabPool.prefabGO is null",
                                                 this.poolName));
    ...
    public partial class PrefabPool
    {
        ...
        public bool logMessages            // Read-only
        {
            get
            {
                ...
                if (spawnPool != null && this.spawnPool.logMessages)
        ...
        {
            if (this.logMessages)
                UnityEngine.Debug.Log(string.Format("SpawnPool {0} ({1}): Despawning '{2}'",
        ...
        {
            if (this.logMessages)
                UnityEngine.Debug.Log(string.Format("SpawnPool {0} ({1}): CULLING TRIGGERED! " +
            ...
            while (this.totalCount > this.cullAbove)
            {
                ...
                for (int i = 0; i < this.cullMaxPerPass; i++)
                {
                    ...
                    if (this._despawned.Count > 0)
                    {
                        ...
                        if (this.logMessages)
                            UnityEngine.Debug.Log(string.Format("SpawnPool {0} ({1}): " +
            ...
            if (this.logMessages)
                UnityEngine.Debug.Log(string.Format("SpawnPool {0} ({1}): CULLING FINISHED! Stopping",
                                        this.spawnPool.poolName,
                                        this.prefab.name));
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
            ...
            else
            {
                ...
                if (this.logMessages)
                    UnityEngine.Debug.Log(string.Format("SpawnPool {0} ({1}): respawning '{2}'.",
                                            this.spawnPool.poolName,
                                            this.prefab.name,
                                            inst.name));
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            if (this.logMessages)
                UnityEngine.Debug.Log(string.Format("SpawnPool {0} ({1}): Spawned new instance '{2}'.",
                                        this.spawnPool.poolName,
                                        this.prefab.name,
                                        inst.name));
        ...
        internal void PreloadInstances()
        {
            ...
            if (this.prefab == null)
            {
                UnityEngine.Debug.LogError(string.Format("SpawnPool {0} ({1}): Prefab cannot be null.",
                                             this.spawnPool.poolName,
                                             this.prefab.name));
            ...
            {
                UnityEngine.Debug.LogWarning(string.Format("SpawnPool {0} ({1}): " +
