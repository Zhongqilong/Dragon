...
public class CardScrollView : MonoBehaviour
{
    ...
    public InputSystemType inputType = InputSystemType.UGUIInput;
    ...
    private CardScrollItem curCenterItem;
    private CardScrollItem preCenterItem;
    ...
    public List<CardScrollItem> listEnhanceItems;
    // sort to get right index
    private List<CardScrollItem> listSortedItems = new List<CardScrollItem>();

    private static CardScrollView instance;
    public static CardScrollView GetInstance
    {
        get { return instance; }
    }
    ...
    void Start()
    {
        ...
        for (int i = count - 1; i >= 0; i--)
        {
            ...
            listEnhanceItems[i].SetSelectState(startCenterIndex == i);
            GameObject obj = listEnhanceItems[i].gameObject;

            UDragCardScrollView script = obj.GetComponent<UDragCardScrollView>();
            if (script != null)
        ...
        // sorted items
        listSortedItems = new List<CardScrollItem>(listEnhanceItems.ToArray());
    ...
    /// 
    /// Update CardScrollItem state with curve fTime value
    /// 
    public void UpdateEnhanceScrollView(float fValue)
    {
        for (int i = 0; i < listEnhanceItems.Count; i++)
        {
            CardScrollItem itemScript = listEnhanceItems[i];
    ...
    private int GetMoveCurveFactorCount(CardScrollItem preCenterItem, CardScrollItem newCenterItem)
    {
        SortEnhanceItem();
    ...
    static public int SortPosition(CardScrollItem a, CardScrollItem b) { return a.transform.localPosition.x.CompareTo(b.transform.localPosition.x); }
    ...
    public void SetHorizontalTargetItemIndex(CardScrollItem selectItem)
    {
        if (!canChangeItem)
            return;
    ...
    public void OnSelectItem(int index)
    {
        if (!canChangeItem)
            return;
        if (curCenterItem.CurveOffSetIndex == index)
            return;
        int targetIndex = index;
        SetHorizontalTargetItemIndex(listEnhanceItems[targetIndex]);
    }

    public void OnDragEnhanceViewBegin(Vector2 delta)
    {
        _EnableBtnInteraction(false);
    }
    ...
    public void OnDragEnhanceViewEnd()
    {
        ...
        canChangeItem = false;
        _EnableBtnInteraction(true);
    }

    private void _EnableBtnInteraction(bool b)
    {
        foreach (var item in listEnhanceItems)
        {
            var v = item.transform.GetComponent<Button>();
            if (v != null)
            {
                v.interactable = b;
            }
