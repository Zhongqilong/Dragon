...
public class ScrollViewEx : MonoBehaviour, IEndDragHandler, IBeginDragHandler
{
    ...
    // public bool test;
    // public Action onHitBottom;
    // public Action onSelectCenter;
    RectTransform _root;
    RectTransform _cell;
    ...
    // private Action<GameObject, int, object> _OnSpawnFunc;
    ...
    public void InitAndFit(Action<Transform, int> func, int fitCount)
    {
        ...
        if (direction == Direction.Horizontal)
        {
            ...
            //rect.sizeDelta = new Vector2(fitCount * (_cell.sizeDelta.x + spacing.x) - spacing.x, _rectTransform.sizeDelta.y);
        }
        else
        {
            rect.SetHeight(fitCount * (_cell.sizeDelta.y + spacing.y) - spacing.y);
            //rect.sizeDelta = new Vector2(_rectTransform.sizeDelta.x, fitCount * (_cell.sizeDelta.y + spacing.y) - spacing.y);
        }
    ...
    void _Init()
    {
        ...
        // _cell = Instantiate(_cell, transform, false);

        _cell = cell.GetComponent<RectTransform>();//_root.childCount > 1 ? _root.GetChild(1) : _cell;
        //Calculate height, width
        //Bounds bounds = UIHelper.CalculateBounds(firstCell);
    ...
    public void Init(Action<Transform, int> func, int count, Action<int> pageChangedAction = null)
    {
        ...
        _Init();
        //_SetCellPos(_cell, 0, 0);
        // var rect = firstCell.rect;
        // _height = cellSize.y > 0 ? cellSize.y : firstCell.sizeDelta.y;
        // _width = cellSize.x > 0 ? cellSize.x : firstCell.sizeDelta.x;
        // //bot = cellSize.y > 0 ? cellSize.y / 2 : -bounds.min.y;
        // _top = cellSize.y > 0 ? cellSize.y / 2 : rect.yMax;
        // _left = cellSize.x > 0 ? cellSize.x / 2 : -rect.xMin;
        // //right = cellSize.x > 0 ? cellSize.x / 2 : bounds.max.x;
        // _centerX = cellSize.x > 0 ? 0 : rect.center.x;
        // _centerY = cellSize.y > 0 ? 0 : rect.center.y;
        ...
        //if (_bestFitSize == 0)
        //{
        //    Uqee.Debug.LogError("Not Enough Space!");
        //    return;
        //}
        ...
        for(int i=_cellList.Count-1; i>=cacheSize; i--)
        {
            ...
            ResManager.DespawnChildren(cell);
    ...
    private void _CreateCell()
    {
        ...
        // curCell.localPosition = _outSizePos;
        ...
        while (_tmpCellList.Count > 0)
        {
            for (; i < _cellList.Count; i++)
            {
                if (_cellList[i] == null)
                {
                    ...
                    // _cellList[i].localPosition = _outSizePos;
                    break;
                }
            }
    ...
    private void _SetCell(Transform cell, bool outSize,int cellIdx, int col, int row, int dataIdx)
    {
        ...
        if (outSize)
        {
            if(cell!=null)
            {
                ...
                // cell.localPosition = _outSizePos;
            }
        }
        else
        {
    ...
    // Vector3 _outSizePos = new Vector3(10000f, 10000f, 10000f);
    ...
    private void MoveRelative(float relative, bool instant = true, float time = 0.3f)
    {
        ...
        else
        {
            ...
            // if (onSelectCenter != null)
            // {
            //     _tweener.onComplete = () => onSelectCenter();
            // }
        }
    }
    ...
    void OnDrawGizmos()
    {
        ...
        if (!Application.isPlaying)
        {
            ...
            // Bounds bounds = UIHelper.CalculateBounds(cell);
