...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        public bool enableManualScroll = true;
        ...
        public RectTransform RecycleNode { get { return _recycledCellViewContainer; } }
        /// <summary>
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab)
        {
            ...
            var cellView = _GetRecycledCellView(cellPrefab.cellIdentifier);
            if (cellView == null)
            {
                var go = Delegate.Instantiate(cellPrefab.cellIdentifier, _container);
        ...
        public EnhancedScrollerCellView GetCellView(string cellIdentifier)
        {
            // see if there is a view to recycle
            var cellView = _GetRecycledCellView(cellIdentifier);
            if (cellView == null)
            {
                var go = Delegate.Instantiate(cellIdentifier, _container);
                cellView = go.GetComponent<EnhancedScrollerCellView>();
                cellView.transform.SetParent(_container);
                cellView.transform.localPosition = Vector3.zero;
                cellView.transform.localRotation = Quaternion.identity;
            ...
            )
        {
            ...
            if (!ignoreVisible && StartDataIndex == 0 && EndDataIndex == (NumberOfCells - 1))
            {
            ...
            if (_posTween != null)
            {
                StopCoroutine(_posTween);
            }
            // start tweening
            _posTween = StartCoroutine(TweenPosition(tweenType, tweenTime, ScrollPosition, newScrollPosition, jumpComplete));
        }
        ...
        #region Extend Custom Functions;
        /// <summary>
        /// 对于Grid类型有问题
        /// </summary>
        /// <param name="dataIndex"></param>
        /// <returns></returns>
        public EnhancedScrollerCellView GetCellViewByDataIndex(int dataIndex)
        {
        ...
        /// <param name="dataIndex"></param>
        /// <returns></returns>
        public void UpdateActiveCells(System.Action<int,Transform> UpdateCellView)
        {
            for (int i = 0; i < _activeCellViews.Count; i++)
            {
                if (_activeCellViews[i] is EnhancedGridCellView)
                {
                    var gridView = (EnhancedGridCellView)_activeCellViews[i];
                    for (int j = 0; j < gridView.rowCellViews.Length; j++)
                    {
                        UpdateCellView(gridView.rowCellViews[j].dataIndex, gridView.rowCellViews[j].transform);
                    }
                }
                else
                {
                    UpdateCellView(_activeCellViews[i].dataIndex, _activeCellViews[i].transform);
                }
            }
        }
        #endregion
        ...
        /// <returns></returns>
        private EnhancedScrollerCellView _GetRecycledCellView(string cellIdentifier)
        {
            for (var i = 0; i < _recycledCellViews.Count; i++)
            {
                if (_recycledCellViews[i].cellIdentifier == cellIdentifier)
                {
        ...
        public GameObject CreateContainerNode()
        {
            var tmpTFM = this.transform.Find("Container");
            if (tmpTFM != null)
            {
                return tmpTFM.gameObject;
            }
        ...
        public GameObject CreateOrGetRecycleNode()
        {
            var tmpTFM = transform.Find("Recycled Cells");
            if (tmpTFM != null)
            {
                // create the recycled cell view container
                return tmpTFM.gameObject;
            }
            var go = new GameObject("Recycled Cells", typeof(RectTransform));
            go.transform.SetParent(transform);
            return go;
        }
